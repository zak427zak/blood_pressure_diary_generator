...
def create_and_configure_window(normal_font, normal_width, small_width, values_dict):
    ...
    high_ad_upper_to_label = Label(start_window, text=' Верхнее, до: ', width=small_width)
    ...
    high_ad_lower_to_label = Label(start_window, text=' Нижнее, до: ', width=small_width)
    ...
    if_standart_checkbutton = Radiobutton(text="Простое заполнение (значения будут разбросаны в случайном порядке)", variable=lang, value=1)
    if_standart_checkbutton.place(x=40, y=530)

    if_need_night_lower_checkbutton = Radiobutton(text="Вечером должно быть ниже (вечерняя пара АД будет всегда ниже, чем утренняя)", value=2, variable=lang)
    if_need_night_lower_checkbutton.place(x=40, y=570)
    if_need_night_lower_checkbutton.select()

    if_need_morning_lower_checkbutton = Radiobutton(text="Утром должно быть ниже (утренняя пара АД будет всегда ниже, чем вечерняя)", value=3, variable=lang)
